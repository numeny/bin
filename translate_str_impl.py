    ...
    for k in translate_base.iterkeys():
        ...
        for line in lines:
            ...
            if line.find(before_name) != -1:
                if len(translate_base[k].strip()) != 0:
...
def fetch_supported_lang_from_res(root):
    global supports_lang_list
    r = re.compile(r'values-(.*)')
    for file_name in os.listdir(root):
        ret = r.match(file_name)
        if ret:
            lang = ret.group(1)
...
def do_manual_translate(name, fenchi_list):
    reset_translate_base()
    '''
    fenchi_list = {
        'en-GB':'Participle',
    ...
    for lang in supports_lang_list:
        try:
            if len(fenchi_list[lang].strip()) == 0:
                continue
...
def do_merge_work_for_select():
    ...
    do_sw_video_adjust_brightness('sw_video_adjust_brightness')
    write_to_output(chromium_root + 'sogou_webview/java/res/', 'sw_video_error_text_unknown')

    do_sw_video_adjust_volume('sw_video_adjust_volume')
    write_to_output(chromium_root + 'sogou_webview/java/res/', 'sw_video_adjust_brightness')

    do_sw_video_guesture('sw_video_guesture')
    write_to_output(chromium_root + 'sogou_webview/java/res/', 'sw_video_adjust_volume')

def write_to_res_output(append_name):
    chromium_root = '/home/bdg/ssd/chromium_68.0.3440.106/src/'
    write_to_output(chromium_root + 'sogou_webview/java/res/', append_name)
...
def init():
    ...
    fetch_supported_lang_from_res('/home/bdg/ssd/chromium_68.0.3440.106/src/sogou_webview/java/res/')
